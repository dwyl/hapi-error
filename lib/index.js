  ...
  server.ext('onPreResponse', function (request, reply) {
    ...
    if (res.isBoom) {
      ...
      if (config.statusCodes[statusCode]
        ...
        let redirectPath = config.statusCodes[statusCode].redirect;
        if (typeof config.statusCodes[statusCode].redirect === 'function') {
          redirectPath = config.statusCodes[statusCode].redirect(request);
        }
        if (redirectPath !== null) {
          return reply.redirect(redirectPath + '?redirect=' + request.url.path);
        }
      }
