  ...
  server.ext('onPreResponse', function (request, reply) {
    ...
    if (res.isBoom) {
      ...
      if (config.statusCodes[statusCode]
        ...
        let redirect = config.statusCodes[statusCode].redirect;
        if (typeof config.statusCodes[statusCode].redirect === 'function') {
          redirect = config.statusCodes[statusCode].redirect(request);
        }
        // if null just do default way
        if (redirect === null) {
          return reply.redirect(redirect + '?redirect=' + request.url.path);
        } else if (redirect !== false) { // if not false redirect to the result
